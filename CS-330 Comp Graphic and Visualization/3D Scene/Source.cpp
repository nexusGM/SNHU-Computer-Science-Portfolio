 ...
 * 
 * 
 * 7-1 Final Project: 3D Scene in Visual Studio
 * 
 * 
****************************************************/
...
//window title
const char* const WINDOW_TITLE = "7-1 Final Project: 3D Scene in Visual Studio | Matthew Tyson";
...
// Variable to handle changing perspectives:
...
void UProcessInput(GLFWwindow* window)
{
	...
	// Increase color intensity (1, 2, 3)
	...
	// Turn off orbit light -=
	if (glfwGetKey(window, GLFW_KEY_MINUS) == GLFW_PRESS)
	{
		gSpotLightColor.r = 0.0f;
		gSpotLightColor.g = 0.0f;
		gSpotLightColor.b = 0.0f;
	...
	{
		gSpotLightColor.r = 1.0f;
		gSpotLightColor.g = 1.0f;
		gSpotLightColor.b = 1.0f;

	}

	// Press and hold P to switch to Ortho mode.
...
void URender(vector<GLMesh> scene)
{
	...
	// When P is held, Ortho is toggled:
	if (!toggleOrtho) {
	...
	for (auto i = 0; i < scene.size(); ++i)
	{
		...
		// Draws the shape
		glDrawArrays(GL_TRIANGLE_STRIP, 0, mesh.nIndices);
